...
class CustomizedGPT2Attention(GPT2Attention):
    """
    Customized GPT-2 attention module.

    This class inherits from `GPT2Attention` but overrides the forward method to
    customize the attention mechanism, potentially integrating flash attention
    and handling padding tokens.
    """

    def __init__(self, *args, **kwargs):
        # Initialize the parent GPT2Attention class
    ...
    def forward(
        ...
        **kwargs,
        ...
        if input_text is not None:
            for cached_prefix, (cached_key, cached_value) in all.items():
                if input_text.startswith(cached_prefix):

                    return None, (cached_key, cached_value)
        ...
        if input_text is not None and use_cache:
            all[input_text] = (key, value)
...
class CustomizedGPT2Block(GPT2Block):
    """
    Customized GPT-2 transformer block.

    This class replaces the standard attention module with the customized version.
    """
...
class CustomizedGPT2Model(GPT2Model):
    ...
    def __init__(self, config):
        ...
        assert self._attn_implementation == 'eager', (
            "[NLPDL ERROR] set _attn_implementation to either 'eager' or 'faster_cache' in this version"
        )
